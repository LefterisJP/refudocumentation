...
* Introduction
This is currently a work in progress. Few things are final, or decided but the purpose
of this document is to gather everything in one place so that the formal specification of
refulang slowly takes shape.

** Format of this document
This document is composed of various sections describing the features and the behaviour of the language. The goal is to serve
as a spec for the implementation of refulang. Sections that don't have any tag should have in princile been implemented.
The remaining sections can have one of the remaining tags:

- **Not_Implented**: This feature has not yet been implemented at all and you won't find any mention of it in the codebase
- **In_Progress**: This feature is currently in progress and is being worked on.
- **Syntax_Only**: This feature is not really implemented, but the scanning and parsing part of the language is aware of it.
                   So syntatictally the feature exists but will fail at analysis and/or backend code generation.
* Built-in data types
...
* Ownership                                                     :In_Progress:
In refulang there exists the concept of ownership. The compiler tries to determine how each object is used and creates an
ownership graph. Studying this graph the compiler decides how to allocate an object (in the memory of in the heap) and 
can detect if the object is properly shared between functions or if there is a problem.

#+BEGIN_SRC refu
type person {name:string, id:u32}

fn createp(name:string) -> person{
    p:person = person(name, 1)
    return p
}

fn printp(name:string) {
    p:person = person(name, 1)
    print(p)
}
#+END_SRC

In the above code examples in function =createp()= the =p= object will be allocated on the heap since it needs to be returned,
while on the =printp()= function the =p= object will only be allocated on the stack since it's only owned by the function.

#+BEGIN_SRC refu
type person {name:string, id:u32}
type couple {husband:person, wife:person}
...
// borrows a person object in a read only manner
fn rusep(p:&person) {
    print(p)
}
...
// obtain ownership of the passed object
fn gusep(p:person) {
    otherp:person = person("Lefteris ", 15)
    c:couple = couple(p, otherp)
    // do something with the c couple object, which now owns the passed person
    gusec(c)
}
...
fn main() {
    ...
    rusep(p)
    guse(p)
    // can't use p after here
}
#+END_SRC

The above code shows how ownership can be transferred across functions. Function =rusep()= can take person objects
in a read only manner. It can't modify said objects. The lifetime of the =p= person object starts with its creation from
=createp()= and continues onto being acquired by the =main()= function.

Afterwards it is passed onto =rusep()= in a read only manner. Then the ownership is essentially transferred inside
=guse()=. There the p object is moved inside a couple object.

Any use of the =p= object inside =main()= after that is illegal.

As for the =c= couple object it's passed onto the =gusec()= function which then owns it. At the end of the function the =c=
couple object will be destroy and =p= along with it.
...
finally by an arrow pointing to the return value.

** Return value
As mentioned, whatever follow =->= is the function's return value.
If there is no return value then the arrow is omitted. Some examples follow:
...
copied inside the function. Objects of all user defined types are passed by reference, which means that simply a pointer to the object
is passed along in the function. TODO: Some more details and optimization here.

* Type Parameters                                               :Syntax_Only:
...
* Macros                                                    :Not_Implemented:
Macros are heavily inspired by the way [[http://docs.racket-lang.org/guide/macros.html][racket]] does macros, offering the ability to totally customize and add new constructs
...
#+END_SRC
** Design By Contract                                      :Not_Implemented:
...
* For expressions                                           :Not_Implemented:
...
be acceptable.
* Parallel Processing Framework / Parallel Routines         :Not_Implemented:
...
* Refu IR
Refu compiles to an intermediate format which then gets translated to the various backends
that may exist. The compiled IR is what objects files are to C/C++ type of languages. A
complete description of the IR can be found [[file:ir.org][here]].
